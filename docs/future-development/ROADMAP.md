...
flowchart LR
    subgraph V110["v1.10.0+ (Current Production Baseline)"]
        direction TB
        B1["Level 3.10 Governed Harness:<br/>• Arize Phoenix tracing (:6006)<br/>• 72 node:test unit tests (9 suites)<br/>• DeclarativeWorkflowEngine<br/>• 4 pre-compiled MCP servers<br/>• Google Antigravity ('agy') search<br/>• Hardened read-only sandbox"]
    end
