...
* **✋ Human approval gates** — host-signed Ed25519 approval tokens (`./dsh.sh approve`) with state-bound, single-consumption checkpoints for workflows that need a person in the loop.
...
> [!NOTE]
> **No external registration is required for approval keys.** `./dsh.sh` creates the host Ed25519 keypair locally; only the public key is mounted into the runtime. `DSH_APPROVAL_SECRET` remains a non-production compatibility fallback.
>
